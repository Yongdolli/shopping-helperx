...
폰·패드·PC 공용 PWA + Python 수집 워커 + Supabase. 현재 v1.0.
...
- **평소보다 싼 딜** (`worker/market.py`, 011 `market_prices` + deals.ref_price/ref_name/ref_url/below_pct/ref_checked): 새 딜(3일 이내, 가격 있음)을 실행당 15건,
  ...
  · 쿠팡(파트너스 API, 키 있을 때만, **다른 소스가 못 찾은 딜만, 실행당 3회** — 추적 상품 어댑터 5회와 합쳐 시간당 8회: 계정당 ~10회/시간 초과 시 403·정지 위험). 모든 소스가 응답 실패면 ref_checked 를 두지 않아 다음 실행에서 재시도. 해외 통화 상점 가격은 원화 딜과 비교 안 함.
...
- 쿠팡: 상품 ID 조회 API 가 없어 **상품명 검색 → productId 매칭**. 등록 시 상품명 필수. 실행당 5회(딜 시세 3회와 공유), 6시간 간격.
